...
public class AdminMainFrame extends JFrame {
    ...
    private void initUi() {
        ...
        // Students menu with Refresh option
        JMenu studentsMenu = new JMenu("Students");
        JMenuItem refreshStudentsItem = new JMenuItem("Refresh list");
        refreshStudentsItem.addActionListener(e -> {
            if (studentsPanel != null) {
                studentsPanel.loadStudents();
            }
        });
        studentsMenu.add(refreshStudentsItem);
        menuBar.add(studentsMenu);
        setJMenuBar(menuBar);

        JTabbedPane tabs = new JTabbedPane();
        tabs.addTab("Users", new AdminUsersPanel());
        studentsPanel = new StudentsPanel();
        tabs.addTab("Students", studentsPanel);
        tabs.addTab("Instructors", new AdminInstructorsPanel());

        // Auto-refresh students list whenever the Students tab is selected
        tabs.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                if (tabs.getSelectedComponent() == studentsPanel) {
                    studentsPanel.loadStudents();
                }
            }
        });
        tabs.addTab("Courses", new CoursesPanel());
        tabs.addTab("Sections", new AdminSectionsPanel());
        tabs.addTab("Maintenance", new AdminMaintenancePanel());
    ...
    private void updateMaintenanceBanner() {
        if (AccessControl.isMaintenanceOn()) {
            maintenanceLabel.setText("Maintenance Mode is ON - read-only for non-admins");
        } else {
